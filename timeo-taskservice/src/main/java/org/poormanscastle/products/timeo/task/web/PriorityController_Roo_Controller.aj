...
import org.poormanscastle.products.timeo.task.web.PriorityController;
...
privileged aspect PriorityController_Roo_Controller {
    ...
    public String PriorityController.show(@PathVariable("id") String id, Model uiModel) {
    ...
    public String PriorityController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("prioritys", Priority.findPriorityEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Priority.countPrioritys() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("prioritys", Priority.findAllPrioritys(sortFieldName, sortOrder));
        }
    ...
    public String PriorityController.updateForm(@PathVariable("id") String id, Model uiModel) {
        populateEditForm(uiModel, Priority.findPriority(id));
        return "prioritys/update";
    }
    
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = "text/html")
    public String PriorityController.delete(@PathVariable("id") String id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
